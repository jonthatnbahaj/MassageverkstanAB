  ...
  return (
    <AnimatePresence>
      <motion.div 
        ...
        exit="exit"
      >
        {/* Minimized Header with Massageverkstan branding */}
        <motion.div 
          className="bg-gradient-to-r from-brand-primary via-brand-secondary to-brand-primary text-white px-4 py-2 flex items-center justify-between shadow-lg relative z-[10000] h-12 flex-shrink-0"
          variants={headerVariants}
        >
          <div className="flex items-center min-w-0 flex-1">
            <motion.img 
              src={businessConfig.logo} 
              alt="Massageverkstan Logo" 
              className="w-6 h-6 mr-2 rounded-full bg-white p-0.5 flex-shrink-0 border border-brand-accent"
              whileHover={{ rotate: 360, scale: 1.1 }}
              transition={{ duration: 0.6 }}
            />
            <div className="min-w-0 flex-1">
              <motion.h2 
                className="font-bold text-xs truncate text-brand-accent"
              ...
              <motion.p 
                className="text-xs opacity-90 flex items-center truncate text-brand-lightGold"
                initial={{ x: -20, opacity: 0 }}
        ...
        >
          {/* Loading State with Massageverkstan branding */}
          <AnimatePresence>
            {isLoading && (
              <motion.div 
              ...
              >
                <div className="text-center px-4">
                  <motion.div 
                    className="w-10 h-10 border-b-2 border-brand-primary rounded-full mx-auto mb-3"
                    animate={{ rotate: 360 }}
                    transition={{ 
                      duration: 1,
                      repeat: Infinity,
          ...
          {/* Offline State with Massageverkstan branding */}
          <AnimatePresence>
            {!isOnline && (
              <motion.div 
              ...
              >
                <div className="text-center max-w-sm">
                  ...
                  <div className="space-y-3">
                    <motion.button
                      onClick={handleRetry}
                      className="w-full bg-brand-primary text-white px-6 py-3 rounded-lg font-bold hover:bg-brand-secondary transition-colors disabled:opacity-50"
                      disabled={!isOnline}
          ...
          {/* Error State with Massageverkstan branding */}
          <AnimatePresence>
            {hasError && isOnline && (
              <motion.div 
              ...
              >
                <div className="text-center max-w-sm">
                  ...
                  <p className="text-gray-600 mb-4 text-sm">
                    Detta kan bero på webbläsarens säkerhetsinställningar eller tillfälliga problem med bokningssystemet. 
                    Försök igen eller öppna bokningen i din webbläsare.
                  </p>
                  <div className="space-y-3">
                    <motion.button
                      onClick={handleRetry}
                      className="w-full bg-brand-secondary text-white px-6 py-3 rounded-lg font-bold hover:bg-brand-primary transition-colors"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      Försök igen
                    ...
                    <motion.button
                      ...
                      className="w-full bg-brand-primary text-white px-6 py-3 rounded-lg font-bold hover:bg-brand-dark transition-colors"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      Öppna i webbläsare
          ...
          {isOnline && (
            <motion.iframe
              ...
              title={`Säker bokning - ${serviceName} - Massageverkstan`}
              loading="lazy"
              // iOS Safari specific attributes
              allow="payment; geolocation"
              referrerPolicy="strict-origin-when-cross-origin"
              // Accessibility
              aria-label={`Bokningsformulär för ${serviceName} hos Massageverkstan`}
              initial={{ opacity: 0 }}
